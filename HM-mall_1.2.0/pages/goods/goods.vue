	...
	<view>
		...
		<view class="header">
			...
			<view class="before" :style="{ opacity: 1 - afterHeaderOpacity, zIndex: beforeHeaderzIndex }">
				...
				<view class="icon-btn">
					...
					<view class="icon cart" @tap="buy"></view>
				</view>
			</view>
			<!-- 头部-滚动渐变显示 -->
		...
		<view class="footer">
			...
			<view class="btn">
				...
				<view class="buy" @tap="buy">马上下单</view>
			</view>
		</view>
		<!-- share弹窗 -->
			...
			<view class="layer" @tap.stop="discard">
				<view class="content">
					<view class="row">
						...
						<view class="description">{{roomData.sraddress}}{{roomData.officeaddress}}</view>
					</view>
				</view>
		...
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="discard" @tap="hideSpec">
			...
			<view class="layer" @tap.stop="discard">
				<view class="content">
					...
					<view class="sp">
						<view v-for="(item,index) in spec" :class="[index==selectSpec?'on':'']" @tap="setSelectSpec(index)" :key="index">{{item.price}}/{{item.time}}</view>
					</view>
		...
		<view class="info-box goods-info">
			...
			<view class="title">
				{{roomData.srname}}{{roomData.officename}}
			</view>
		</view>
		<!-- 服务-规则选择 -->
		<view class="info-box spec">
			<view class="row" @tap="showService">
				...
				<view class="content"><view class="serviceitem">{{roomData.region}}</view></view>
			...
			<view class="row" @tap="showSpec(false)">
				...
				<view class="content">
					<view>类型：</view>
					<view class="sp">
						<view v-for="(item,index) in spec" :key="index" :class="[index==selectSpec?'on':'']">{{item.time}}</view>
					</view>
				</view>
				<view class="arrow"><view class="icon xiangyou"></view></view>
			</view>
...
export default {
	data() {
		return {
			...
			//轮播主图数据
			swiperList: [
				{ id: 1, img: '/static/img/c1.jpg' },
				{ id: 2, img: '/static/img/c1.jpg' },
				{ id: 3, img: '/static/img/c1.jpg' },
				{ id: 4, img: '/static/img/c1.jpg' }
			],
			//轮播图下标
			currentSwiper: 0,
			anchorlist:[],//导航条锚点
	...
	onLoad(option) {
		...
		if(option.srid == undefined) {
			this.type = '1';
			uni.request({
			    url: 'http://zy.eatclub.wang:3000/list/office/detail?id=' + option.officeid,
			    method: 'GET',  
			    data: {},  
			    success: res => {  
					// console.log(res.data);
					this.roomData = res.data.data[0];
					this.spec = [
						{time:"月",price:res.data.data[0].price},
						{time:"年",price:res.data.data[0].price*8}
					]
			    },  
			    fail: () => {},  
			    complete: () => {}
			...
			this.type = '0';
			uni.request({
			    url: 'http://zy.eatclub.wang:3000/list/sr/detail?id=' + option.srid,
			    method: 'GET',  
			    data: {},  
			    success: res => {  
					// console.log(res.data);
					this.roomData = res.data.data[0];
					this.spec = [
						{time:"小时",price:res.data.data[0].price/5},
						{time:"日",price:res.data.data[0].price},
						{time:"月",price:res.data.data[0].price*24}
					]
			    },  
			    fail: () => {},  
			    complete: () => {}
	...
	methods: {
		...
		keep(){
			...
			if(this.type == '0') {
				if(this.isKeep) {
					uni.request({
					    url: 'http://zy.eatclub.wang:3000/addcollect',
					    method: 'GET',  
					    data: {
							roomid:this.roomData.srid,
		...
		toConfirmation(){
			let tmpList={
				...
				spec:this.spec[this.selectSpec],
				number:this.number,
			};
			uni.setStorage({
				key:'order',
